...
| — | **DECLINED** docx / PDF export tooling (owner, 2026-09-03) | No `python-docx`, no LibreOffice, no browser automation, no download link or blob save (the Claude Artifact viewer blocks page-initiated downloads). The HTML carries a "Save as PDF" button that only calls `window.print()`, with a ⌘P / Ctrl+P hint because print may be restricted in that viewer; documented as "HTML report → Save as PDF (browser print)" | Owner: "ทำเป็น HTML หรือ artifact ก็ได้ ไม่ต้องทำเป็น docx+PDF", then "ทำปุ่มโหลดเป็น PDF ได้ไหม" — yes via the browser, at zero cost |
