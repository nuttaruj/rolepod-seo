...
check("prefers-color-scheme: dark" not in doc and "data-theme" not in doc and "color-scheme:light" in doc and "--bg:#FFFFFF" in doc, "single light theme: white page, no dark overrides")
...
check(all(e.startswith(allowed) for e in ext) and len(ext) == 1, f"the only external request is the Google Fonts stylesheet (found {ext[:3]})")
